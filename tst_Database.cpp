...
TEST_CASE("parse calculator index line", "[index]") {
    auto calculator_stream = std::istringstream(calculator);
    const Index index = loadIndex(calculator_stream);
...
TEST_CASE("skip license header", "[index]") {
    auto header_stream = std::istringstream(licenseHeader);
    const Index index = loadIndex(header_stream);
    REQUIRE(index.empty());
}
